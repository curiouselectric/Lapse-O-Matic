...
/* ********* ESP32_CAM Wildlife Camera Unit **************************
    ...
    if no RTC: need to sort out naming as it's overwriting the same one.
...
void setup()
{
  ...
  if (filename == "NO_RTC_")
  {
    filename = filename + (String)PIC_COUNT + "_";
  }
  ...
  if (err != ESP_OK) {
    ...
    {
      DEBUGLN(settings_config.DEBUG_FLAG, ("Error at : " + filename + " Code: " + err));
      // Write to file
      // If we're here, the file was created. Now write the captured image to the file.
      file.println("Error at : " + filename );
    }
...
void loop()
{
  ...
  while (COUNTUP <= settings_config.NUMBER_PHOTOS)
  {
    ...
    String path = "/" + filename + "_" + String(COUNTUP) + "_of_" + String(settings_config.NUMBER_PHOTOS) + ".jpg";

    DEBUGLN(settings_config.DEBUG_FLAG, ("Path is: " + path));
